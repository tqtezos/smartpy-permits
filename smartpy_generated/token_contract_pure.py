...
class FA12_core(sp.Contract):
    def __init__(self, **extra_storage):
        ...
        with open('metadata/metadata.json', 'r') as f:
          ...
          self.init(balances=sp.big_map(tvalue=sp.TRecord(approvals=sp.TMap(sp.TAddress, sp.TNat), balance=sp.TNat)), totalSupply=0,
                    ...
                    max_expiry = 2628000), metadata=sp.big_map(l={"": sp.bytes_of_string("tezos-storage:md-json"), "md-json": sp.bytes_of_string(metadata)}), **extra_storage)
    ...
    def getEffectiveExpiry(self, params):
        ...
        with sp.if_(self.data.permit_data.permit_expiries.contains(params) & self.data.permit_data.permit_expiries[params].is_some()):
            permit_expiry = self.data.permit_data.permit_expiries[params].open_some()
            sp.result(permit_expiry)
        with sp.else_():
            with sp.if_(self.data.permit_data.user_expiries.contains(address) & self.data.permit_data.user_expiries[address].is_some()):
    ...
    def delete_permit(self, permit_key):
        ...
        with sp.if_(self.data.permit_data.permits.contains(permit_key)):
            del self.data.permit_data.permits[permit_key]
        with sp.if_(self.data.permit_data.permit_expiries.contains(permit_key)):
    ...
    def transfer_presigned(self, params):
        ...
        with sp.if_(self.data.permit_data.permits.contains(permit_key)):
            ...
            with inline(self.getEffectiveExpiry, permit_key) as effective_expiry:
              ...
              with sp.if_(sp.as_nat(sp.now - permit_submission_timestamp) >= effective_expiry):
                  # Expired
...
class FA12_mint_burn(FA12_core):
    ...
    def mint(self, params):
        ...
        sp.verify(self.is_administrator(sp.sender), self.error_message.user_unauthorized())
...
if "templates" not in __name__:
    ...
    def test():
        ...
        scenario += c1.getAllowance(sp.record(alice.address, bob.address), view_allowance.typed)
        scenario.verify_equal(view_allowance.data.last, sp.some(1))

        scenario.h2("Counter")
        view_counter = Viewer(sp.TNat)
        scenario += view_counter
        scenario += c1.getCounter(sp.unit, view_counter.typed)
        ...
        scenario += c1.getDefaultExpiry(sp.unit, view_defaultExpiry.typed)
