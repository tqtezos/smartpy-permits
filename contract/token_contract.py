...
class FA12(sp.Contract):
    ...
    def setExpiry(self, params):
        ...
        sp.if sp.snd(sp.snd(params)).is_some():
            ...
            permit_submission_timestamp = self.data.permits[sp.pair(
                address, some_permit)]
            sp.if self.data.permit_expiries.contains(sp.pair(address, some_permit)) & self.data.permit_expiries[sp.pair(address, some_permit)].is_some():
